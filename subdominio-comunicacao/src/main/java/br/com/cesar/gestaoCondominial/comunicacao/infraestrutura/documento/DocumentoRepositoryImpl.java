...
public class DocumentoRepositoryImpl implements DocumentoRepository {
    ...
    public void save(Documento d) {
        boolean existe = Boolean.TRUE.equals(
            jdbcTemplate.queryForObject(
                "SELECT COUNT(*) > 0 FROM documentos WHERE id = ?",
                Boolean.class, d.getId().getValor()
            )
        );
        if (!existe) {
            jdbcTemplate.update(
    ...
    public List<Documento> findAtivos() {
        return jdbcTemplate.query(
            "SELECT * FROM documentos WHERE status = 'ATIVO' ORDER BY nome",
            rowMapper
        );
    }
    ...
    public List<Documento> findAll() {
        return jdbcTemplate.query(
            "SELECT * FROM documentos ORDER BY nome",
            rowMapper
        );
    }
}
