...
public class GestaoDeDocumentosSteps extends DominiumFuncionalidade {
    ...
    public void o_sistema_incrementa_o_numero_da_versao_do_documento() {
        Optional<VersaoDocumento> ultima = versaoDocumentoRepository.findUltimaVersao(ultimoDocumentoSalvo.getId());
        assertTrue(ultima.isPresent() && ultima.get().getVersao() > 1);
    }

    @Then("o histórico mantém a versão anterior armazenada")
    public void o_historico_mantem_a_versao_anterior_armazenada() {
        List<VersaoDocumento> versoes = versaoDocumentoRepository.findHistorico(ultimoDocumentoSalvo.getId());
        assertTrue(versoes.stream().anyMatch(v -> v.getVersao() == 1));
        assertTrue(versoes.stream().anyMatch(v -> v.getVersao() == 2));
    }
